...
(defun nslookup (hostname)
   ...
   (if hostname
       (host-ent-address (get-host-by-name hostname)) nil))

;; open a server port

(defun tcp-server (&key (interface (machine-instance)) (port 8000) (backlog 5))
   "Returns a socket server bound to PORT on INTERFACE.  Defaults to (machine-instance):8000. The queue size BACKLOG defaults to 5 connections."
   (handler-case
...
(defun tcp-accept (server &key (timeout 30))
   ...
   A new socket is returned, which is bound to the client connection.  If a timeout occured, nil is returned."
   (when server
